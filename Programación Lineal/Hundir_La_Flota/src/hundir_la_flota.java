...
public class hundir_la_flota {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		// El jugador pone 2 portaaviones(4 casillas), 3 buques(2 casillas) y 4
		// lanchas(1 casilla)
		for (g = 1; g < 3; g++) {
			for (i = 0; i < 4; i++) {
				posiciones[i] = "0";
			...
			/*
			 * El formato de posición es letra+número, otro tipo de dato introducido no se
			 * permitirá, el valor máximo de letra será J y el valor máximo de número será
			 * 10
			 */
			do {
				Ypos = 0;
				...
				if (posicion.length() < 2 || posicion.length() > 2 && !posicion.substring(1).equals("10")
						|| fila.indexOf(posicion.charAt(0)) == -1 || !Character.isDigit(posicion.charAt(1))) {
				...
				/*
				 * Comprueba que en el eje vertical y en el eje horizontal quepa el portaaviones
				 */
				for (i = 0; i < 4; i++) {
					if ((Y + 3) < 11) {
			...
			} while ((posicion.length() < 2 || posicion.length() > 2 && !posicion.substring(1).equals("10")
					|| fila.indexOf(posicion.charAt(0)) == -1 || !Character.isDigit(posicion.charAt(1))
					|| !tablero[Y][X].equals("|  |")) || (Ypos == 1 && Yneg == 1 && Xpos == 1 && Ypos == 1));

			/*
			 * Guarda las posiciones posibles donde poner la otra punta del portaaviones
			 */
			if (Xpos == 0) {
			...
			do {
				...
				/*
				 * Pide donde colocar la otra punta del portaaviones indicando las posiciones
				 * posibles
				 */
				...
				/*
				 * Esto se repite hasta que el jugador ponga una de las posiciones posibles
				 * indicadas
				 */
			} while (validacion != "valido");

			/*
			 * Si se selecciona una de las posibles posiciones se coloca el portaaviones
			 */
			for (i = 0; i < 4; i++) {
		...
		/*
		 * buques (El código es igual al del portaaviones pero ajustado al tamaño del
		 * buque
		 */
		for (g = 1; g < 4; g++) {
			for (i = 0; i < 4; i++) {
		...
		for (g = 1; g < 3; g++) {
			...
			do {
				...
				Xpos = 0;
				Xneg = 0;
				posicion = fila.charAt((int) (Math.random() * (11 - 1) + 1))
						+ String.valueOf((int) (Math.random() * (11 - 1) + 1));
				Y = fila.indexOf(posicion.charAt(0));
				X = Integer.parseInt(posicion.substring(1));
				for (i = 0; i < 4; i++) {
					if ((Y + 3) < 11) {
			...
			/*
			 * Elige aleatoriamente una de las 4 posibles posiciones para la otra punta
			 */
			do {
		...
		/*
		 * buques, el código es igual al de portaaviones ajustado para el tamaño de los
		 * buques
		 */
		for (g = 1; g < 4; g++) {
			for (i = 0; i < 4; i++) {
		...
		while (barcos_hun < 9 && barcos_hunr < 9) {
			...
			case 0:
				...
				do {
					...
					// Guardo la posicion en 2 enteros para
						...
						|| fila.indexOf(posicion.charAt(0)) == -1 || !Character.isDigit(posicion.charAt(1))
						|| !tablero2[Y][X].equals("| |") && !tablero2[Y][X].equals("|  |"));
				contador = 0;
				...
				if (!tableromaq[Y][X].equals("| |")) {
					...
					/*
					 * Compruebo que tipo de barco es por la letra con la que comienza
					 */
					...
					if (tableromaq[Y][X].startsWith("|L")) {
						...
						for (g = 1; g < 3; g++) {
							if (tableromaq[Y][X].endsWith(String.valueOf(g))) {
								...
								/*
								 * Compruebo las posiciones en vertical y horizontal y guardo en un contador las
								 * cantidad de veces que se ha tocado el portaaviones
								 */
								for (i = 1; i < 4; i++) {
									if ((Y + i) < 11) {
							...
							/*
							 * Compruebo ayudandome del contador si se ha hundido el barco que se ha tocado
							 */
							if (contador == 3) {
								System.out.print(" y hundido!");
								barcos_hun++;
						...
						for (g = 1; g < 4; g++) {
							if (tableromaq[Y][X].endsWith(String.valueOf(g))) {
								...
								/*
								 * Compruebo las posiciones en vertical y horizontal y guardo en un contador las
								 * cantidad de veces que se ha tocado el buque
								 */
								for (i = 1; i < 3; i++) {
									if ((Y + i) < 11) {
			...
			case 1:
				do {
					...
					/*
					 * Reaprobecho la variable g para controlar el flujo más adelante
					 */
					g = 0;

					/*
					 * La máquina comprueba si ha tocado algun barco en los anteriores turnos
					 */
					for (i = 1; i < MAXI; i++) {
						for (j = 1; j < MAXJ; j++) {
							if (tableromaq2[i][j].equals("|T|")) {
								...
								/*
								 * Asigno el valor 10 a "g" si se ha encontrado algún barco tocado para luego
								 * evitar que genere una posición aleatoria
								 */
								g = 10;

								/*
								 * Guarda las posiciones, siempre que sean validas, de el eje vertical y el
								 * horizontal, en caso de haber dos tocados seguidos comprueba si estan en
								 * vertical o horizontal y lo marca
								 */
								if (i + 1 < 11 && (Ypos != 1)) {
								...
								/*
								 * Reinicio las siguientes variables que reutilizaré para controlar el flujo
								 */
								Yneg = 0;
								Ypos = 0;
								Xneg = 0;
								...
								/*
								 * Si hay más de un tocado en el tablero de comprobación de la máquina y estan
								 * posicionados verticalmente entrará aqui
								 */
								if (tocado.equals("vertical")) {
									do {

										/*
										 * Ypos será 1 si desde la posicion para arriba es imposible que continue el
										 * barco y Yneg lo mismo para abajo, en caso de que sean los dos 0 se empezará a
										 * probar aleatoriamente en el eje vertical
										 */
										if (Ypos == 1) {
											j2 = 1;
										} else if (Yneg == 1) {
										...
										case 0:
											...
											/*
											 * La máquina prueba las posiciones hacia arriba si es posible
											 */
										...
										case 1:
											/*
											 * La máquina prueba las posiciones hacia abajo si es posible
											 */
											for (i2 = i - 1; i2 > i - 4; i2--) {
												if (i2 < 1) {
													Yneg = 1;
										...
										/*
										 * El bucle continua mientras una de las direcciones sea posible
										 */
									} while (Yneg != Ypos);

									/*
									 * Si ninguna dirección es posible, la máquina marcará horizontal para hacer
									 * pruebas en el otro eje
									 */
									if (Yneg == 1 && Ypos == 1) {
										tocado = "horizontal";
										j--;
								...
								/*
								 * Si hay más de un tocado en el tablero de comprobación de la máquina y estan
								 * posicionados horizontalmente entrará aqui
								 */
								else if (tocado.equals("horizontal")) {
									do {
										/*
										 * Xpos será 1 si desde la posicion para arriba es imposible que continue el
										 * barco y Xneg lo mismo para abajo, en caso de que sean los dos 0 se empezará a
										 * probar aleatoriamente en el eje vertical
										 */
										if (Xpos == 1) {
											j2 = 1;
										...
										case 0:
											/*
											 * La máquina prueba las posiciones hacia la derecha si es posible
											 */
										...
										/*
										 * La máquina prueba las posiciones hacia la izquierda si es posible
										 */
										case 1:
											for (i2 = j - 1; i2 > j - 4; i2--) {
												if (i2 < 1) {
									...
									/*
									 * Si ninguna dirección es posible, la máquina marcará vertical para hacer
									 * pruebas en el otro eje
									 */
									if (Xpos == 1 && Xneg == 1) {
										tocado = "vertical";
								...
								/*
								 * Si la máquina no sabe si el barco esta en vertical o horizontal prueba una de
								 * las cuatro posibles posiciones de ambos ejes
								 */
								else {
									do {
										...
										/*
										 * Cuando una posicion no era posible se guardaba como 0, asi que esta parte se
										 * repite hasta que posicion no sea 0
										 */
							...
							if (tableromaq2[fila.indexOf(posicion.charAt(0))][Integer.parseInt(posicion.substring(1))]
									.equals("| |") && g == 10) {
								break;
							}
						}
					...
					/*
					 * Si g no es igual a 10 significa que no hay ningún barco tocado, asi que se
				...
				if (!tablero[Y][X].equals("|  |")) {
					...
					// portaaviones
						...
						/*
						 * Compruebo cuantas veces han tocado al mismo portaaviones
						 */
						for (g = 1; g < 3; g++) {
							...
							/*
							 * Si han tocado a un portaaviones 4, (el contador no cuenta la ultima posicion
							 * puesta por la máquina) veces significa que lo han hundido
							 */
							if (contador == 3) {
								for (i = 0; i < 4; i++) {
									if ((Y + i) < 11) {
								...
								/*
								 * Incremento un contador de barcos hundidos con el que se determinará quien
								 * gana
								 */
						...
						/*
						 * Compruebo cuantas veces han tocado al mismo buque
						 */
						for (g = 1; g < 4; g++) {
							if (tablero[Y][X].endsWith(String.valueOf(g) + "|")) {
								tablero[Y][X] = "|TB" + g;
							...
							/*
							 * Si han tocado un mismo buque 3 veces (el contador no cuenta la ultima
							 * posicion puesta por la máquina) significa que lo han hundido
							 */
							if (contador == 2) {
								for (i = 0; i < 4; i++) {
									if ((Y + i) < 11) {
				...
				/*
				 * Hago una pausa para que el jugador pueda leer los mensajes
				 */
