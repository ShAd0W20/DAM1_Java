...
public class UserDAO extends ConnectionDAO {
	...
	public ArrayList<User> getAllUsers() {
		...
		try {
			...
			while (rs.next()) {
				User u = new User(rs.getString("dni"), rs.getString("name"), rs.getString("password"),
	...
	public String getUserInfo(String dni) {
		if (!userExists(dni))
			return String.format("%n%n[!] User doesn't exists%n");
		...
		try {
			...
			ResultSet rs = stmt.executeQuery();
			if (rs.next()) {
				return String.format("%-20s %-20s %-20s %-20s %-20s %n%-20s %-20s %-20s %-20s %-20s%n", "DNI", "Name",
						"Email", "Phone", "Address", rs.getString("dni"), rs.getString("name"), rs.getString("Email"),
