...
public class SellsDAO extends ConnectionDAO {
    ...
    public Bill getBillFromUser(String billOwner, String billID) {
        if (!new UserDAO().userExists(billOwner)) return null;
        if (!this.existsBill(billID)) return null;

        Bill bill = new Bill(billOwner);

        try {
            PreparedStatement stmt = this.conn().prepareStatement("SELECT p.id, p.name, p.stock, p.price, p.iva, c.productquantity, c.buydate, b.billid FROM bills b INNER JOIN shoppingcart c ON c.billid = b.billid INNER JOIN products p ON p.id = c.productid WHERE b.billid = ? AND b.userid = ?;");
            ...
            if (rs.next()) {
                ...
                bill.setBillDate(rs.getDate("buydate").toLocalDate());
    ...
    public ArrayList<Bill> getAllBillsFromUser(String billOwner) {
        ...
        try {
            PreparedStatement stmt = this.conn().prepareStatement("SELECT p.id, p.name, p.stock, p.price, p.iva, c.productquantity, c.buydate, b.billid FROM bills b INNER JOIN shoppingcart c ON c.billid = b.billid INNER JOIN products p ON p.id = c.productid WHERE b.userid = ? GROUP BY b.billid, p.id, p.name, p.stock, p.price, p.iva, c.productquantity, c.buydate;");
    ...
    public ArrayList<Bill> getAllBillsFromUserBetweenDates(String billOwner, LocalDate startDate, LocalDate endDate) {
        ...
        try {
            PreparedStatement stmt = this.conn().prepareStatement("SELECT p.id, p.name, p.stock, p.price, p.iva, c.productquantity, c.buydate, b.billid FROM bills b INNER JOIN shoppingcart c ON c.billid = b.billid INNER JOIN products p ON p.id = c.productid WHERE b.userid = ? AND (c.buydate BETWEEN ? AND ?);");
            stmt.setString(1, billOwner);
    ...
    public String getAllProductsStatistics() {
        ...
        try {
            ...
            while (rs.next()) {
                data.append(String.format("%-20s %-20s %-20s%n%-20s %-20s %-20s%n", "Nombre", "Stock", "Ventas", rs.getString("name"), rs.getString("stock"), rs.getString("totalsells")));
            }
    ...
    public String getProductStatistics(String productName) {
        ...
        if(p == null)  return String.format("%n%n[!] Product not found%n");
