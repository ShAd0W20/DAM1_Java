...
import main.java.utils.ExportBillToPDF;
...
public class ShopView {
	...
	public static boolean mainShopMenu() {
		...
		do {
			...
			u = new UserDAO().login(new User(dni, password));
	...
	private static void buyMenu(@NotNull ShoppingCart cart) {
		...
		do {
			...
			while (productStock < productQuantity) {
				productQuantity = ConsoleHelper.readInteger(String.format(
						"%nIntroduce la cantidad deseada actualmente disponemos de [%s] unidades: ", productStock));
			}
	...
	private static void printActualCart(@NotNull ShoppingCart cart) {
		...
		for (ShoppingCartProduct product : cart.getAllProducts()) {
			System.out.printf("\t[%s] %s -> %s€ | %s unidades %n", product.getProduct().getId(),
					product.getProduct().getName(), product.getProduct().getPrice(), product.getQuantity());
		}
		System.out.println();
	}
	...
	private static void modifyProducts(@NotNull ShoppingCart cart) {
		...
		do {
			...
			System.out.printf("%nActualmente tiene [%s] unidades del producto %s%n", product.getQuantity(),
					productName);
			...
			while (productStock < newQuantity) {
				newQuantity = ConsoleHelper.readInteger(
						String.format("%nIntroduce la nueva cantidad deseada actualmente disponemos de [%s] unidades: ",
								productStock));
			}
	...
	private static void printFinalBill(@NotNull ShoppingCart cart, User u) {
		...
		for (ShoppingCartProduct product : cart.getAllProducts()) {
			...
			data += String.format("%-20s %-20s %-20s %-20s %s€%n", product.getProduct().getName(),
		...
		ExportBillToPDF.export(cart, totalPrice, u);
