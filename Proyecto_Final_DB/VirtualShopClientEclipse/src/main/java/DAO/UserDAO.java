		...
		try {
			...
			if (rs.next()) {
				if (BCrypt.checkpw(u.getPassword(), rs.getString("password"))) {
					user = new User(rs.getString("dni"), rs.getString("name"), rs.getString("password"), rs.getString("email"), rs.getString("phone"), rs.getString("address"));
				} else {
					return u;
				}
			}
