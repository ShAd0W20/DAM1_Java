...
public class RegisterSecondGrades {
			...
			if (!studentNameToSearch.equalsIgnoreCase("salir")) {
				while (raf.getFilePointer() < file.length()) {
					...
					if (studentNameToSearch.equals(studentName.trim())) {
						...
						System.out.println("Introduce la nota para Programación Modular");
						student.subjects.get(1).subjectGrade = input.nextInt();
						input.nextLine();

						System.out.println("Introduce el comentario para Programación Modular");
