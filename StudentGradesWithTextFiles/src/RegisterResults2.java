...
public class RegisterResults2 {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		for(int j = 0; j < studentNames.length; j++) {
			System.out.printf("Introduce la nota para la UF de Programació Modular de %s %s %n", studentNames[j][1], studentNames[j][2]);
			studentGrades[j][1] = input.nextInt();
			input.nextLine();
		}
		
		for(int j = 0; j < studentNames.length; j++) {
			for(int k = 2; k < studentGrades[j].length; k++) {
				studentGrades[j][k] = -1;
			}
		}
